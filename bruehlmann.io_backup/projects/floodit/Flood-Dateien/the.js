...
var COLORS = ['blue', 'orange', 'green']; // set number of colors
...
Game.prototype.goBack = function goBack() {
	...
	for (var i = 1; i <= currentRound; i++) {
		try{
			data[i-1] = [readCookie('r'+i).toString(),readCookie('rt'+i).toString()]
		} catch (e) {
	...
	for (var i = 0; i < data.length; i++) {
		...
		hrefString += '&'+key1+'='+value1+'&'+key2+'='+value2;
		

	};
	//alert(hrefString);
	window.location.href = 'http://ww2.unipark.de/uc/study2015/ospe.php?return_tic='+urlParams['tic']+hrefString;
};
	...
	if (this.hasWon()) {
		...
		setTimeout(function() {
			if (me.moveCount <= me.expected) {
				...
				$('#win').modal('show');
				me.expected --; // make it harder
				$('#win-modal-text').text("Can you do it in " + me.expected + " steps?");
				...
				if(me.round == 3) {
					...
					$('#win-modal-text').text("Trial completed. Please return to the survey.");
					$('#win .modal-footer .goOn').hide();
				...
				if(me.round == 3) {
					// show button to return to survey
					// show button to return to survey
					$('#fail-modal-text').text("Trial completed. Please return to the survey.");
					$('#fail .modal-footer .goOn').hide();
